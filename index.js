...
async function startBot() {
  ...
  // AUTO SV (STORY VIEWER)
  sock.ev.on('stories.upsert', async (stories) => {
    for (const story of stories) {
      try {
        const jid = story.key?.participant || story.key?.remoteJid
        await delay(DELAY)
        await sock.readMessages([story.key])
        console.log('👁️ SV:', jid.split('@')[0])
      } catch(e) { console.error('SV error:', e.message) }
    }
  })
  sock.ev.on('messages.upsert', async ({ messages }) => {
    ...
    if (text.startsWith('.reg') && !text.startsWith('.reg all')) {
      if (!isOwner) { await delay(DELAY)
          await sock.sendMessage(from, { text: '❌ Hanya *Owner* yang bisa daftarkan member!' }, { quoted: msg }); return }
      const mentionedJid = msg.message?.extendedTextMessage?.contextInfo?.mentionedJid?.[0]
      if (!mentionedJid) { await delay(DELAY)
          await sock.sendMessage(from, { text: '⚠️ Gunakan format: *.reg @member*' }, { quoted: msg }); return }
      const db = loadDB()
    ...
    if (text === '.reg all') {
      if (!isOwner) { await delay(DELAY)
          await sock.sendMessage(from, { text: '❌ Hanya *Owner* yang bisa pakai cmd ini!' }, { quoted: msg }); return }
      try {
    ...
    if (text.startsWith('.tss')) {
      if (!isOwner) { await delay(DELAY)
          await sock.sendMessage(from, { text: '❌ Cmd ini khusus *Owner* saja!' }, { quoted: msg }); return }
      ...
      if (!jumlah || isNaN(jumlah) || jumlah <= 0) { await delay(DELAY)
          await sock.sendMessage(from, { text: '⚠️ Format: *.tss jumlah*\nContoh: *.tss 5k* / *.tss 100k* / *.tss 1jt*' }, { quoted: msg }); return }
      const db = loadDB()
    ...
    if (text.startsWith('.ts') && !text.startsWith('.tss')) {
      ...
      jumlah = Math.floor(jumlah)
      if (!jumlah || isNaN(jumlah) || jumlah <= 0) { await delay(DELAY)
          await sock.sendMessage(from, { text: '⚠️ Format: *.ts @player jumlah*\nContoh: *.ts @628xxx 5k*' }, { quoted: msg }); return }
      const targetJid = mentionedJid || senderJid
      const db = loadDB()
    ...
    if (text.startsWith('.ks')) {
      ...
      jumlah = Math.floor(jumlah)
      if (!jumlah || isNaN(jumlah) || jumlah <= 0) { await delay(DELAY)
          await sock.sendMessage(from, { text: '⚠️ Format: *.ks @player jumlah*\nContoh: *.ks @628xxx 5k*' }, { quoted: msg }); return }
      const targetJid = mentionedJid || senderJid
      const db = loadDB()
      if (!db[targetJid]) { await delay(DELAY)
          await sock.sendMessage(from, { text: '❌ Player tidak ditemukan di database!' }, { quoted: msg }); return }
      const before = db[targetJid].coin || 0
      if (jumlah > before) { await delay(DELAY)
          await sock.sendMessage(from, { text: '⚠️ Saldo tidak cukup!\n💰 Saldo saat ini: *Rp ' + before.toLocaleString('id-ID') + '*' }, { quoted: msg }); return }
      db[targetJid].coin = before - jumlah
      const after = db[targetJid].coin
      saveDB(db)
      const num = targetJid.split('@')[0]
    ...
    if (text.startsWith('.tf')) {
      const mentionedJid = msg.message?.extendedTextMessage?.contextInfo?.mentionedJid?.[0]
      if (!mentionedJid) { await delay(DELAY)
          await sock.sendMessage(from, { text: '⚠️ Format: *.tf @player jumlah*\nContoh: *.tf @628xxx 5k*' }, { quoted: msg }); return }
      if (mentionedJid === senderJid) { await delay(DELAY)
          await sock.sendMessage(from, { text: '❌ Tidak bisa transfer ke diri sendiri!' }, { quoted: msg }); return }
      const args = text.split(' ')
      const rawNum = args[args.length - 1].toLowerCase()
      let jumlah = 0
      if (rawNum.endsWith('jt')) jumlah = parseFloat(rawNum) * 1000000
      ...
      jumlah = Math.floor(jumlah)
      if (!jumlah || isNaN(jumlah) || jumlah <= 0) { await delay(DELAY)
          await sock.sendMessage(from, { text: '⚠️ Jumlah transfer tidak valid!' }, { quoted: msg }); return }
      const db = loadDB()
      if (!db[senderJid]) { await delay(DELAY)
          await sock.sendMessage(from, { text: '❌ Kamu belum terdaftar! Ketik *.reg* dulu' }, { quoted: msg }); return }
      if (!db[mentionedJid]) { await delay(DELAY)
          await sock.sendMessage(from, { text: '❌ Player tujuan belum terdaftar!' }, { quoted: msg }); return }
